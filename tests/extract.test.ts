...
import { normalizeOptions, optionTexts } from "@/lib/options";
...
describe("extractDraftQuestions", () => {
  it("parses MCQ + subjective questions and applies an embedded answer key", () => {
    ...
    assert.deepEqual(optionTexts(questions[0].options), ["3", "4", "5", "6"]);
    assert.equal(questions[0].correct_answer, "4");

    assert.equal(questions[1].question_type, "mcq");
    assert.deepEqual(optionTexts(questions[1].options), ["Berlin", "Madrid", "Paris", "Rome"]);
  ...
  it("parses a continuous stream with no line breaks (real PDF-style)", () => {
    ...
    assert.deepEqual(optionTexts(questions[1].options), ["Berlin", "Paris", "Rome", "Madrid"]);
  });
  ...
  it("handles Q-prefixed numbering and lowercase parenthesised options", () => {
    ...
    assert.deepEqual(optionTexts(questions[0].options), ["Utility bill", "Passport", "Selfie", "None"]);
  ...
  it("strips difficulty tags, section headers and end-of-paper rules from blocks", () => {
    ...
    assert.equal(questions.length, 2);
    assert.deepEqual(optionTexts(questions[0].options), ["3", "4", "5", "6"]);
    assert.deepEqual(optionTexts(questions[1].options), ["w", "x", "y", "z"]);
  });
  ...
  it("does not let an 'A.' inside the stem hijack the option block", () => {
    ...
    assert.deepEqual(optionTexts(questions[0].options), [
      "First Normal Form",
      "Second Normal Form",
      "Third Normal Form",
      "None"
...
describe("applyAnswerKey", () => {
  it("fills MCQ correct answers by position and leaves other rows untouched", () => {
    const questions: DraftQuestion[] = [
      { question_text: "q1", question_type: "mcq", topic: "General", options: normalizeOptions(["3", "4", "5", "6"]), correct_answer: null, max_marks: 1, rubric: null },
      { question_text: "q2", question_type: "subjective", topic: "General", options: null, correct_answer: null, max_marks: 2, rubric: null },
      { question_text: "q3", question_type: "mcq", topic: "General", options: normalizeOptions(["w", "x", "y", "z"]), correct_answer: null, max_marks: 1, rubric: null }
    ];
  ...
  it("ignores out-of-range letters", () => {
    const questions: DraftQuestion[] = [
      { question_text: "q1", question_type: "mcq", topic: "General", options: normalizeOptions(["3", "4"]), correct_answer: null, max_marks: 1, rubric: null }
    ];
